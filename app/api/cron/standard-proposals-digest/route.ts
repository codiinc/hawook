...
import { renderDailyStandardDigest } from '@/lib/email-templates/daily-standard-digest'
...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    console.error('[standard-proposals-digest] query error:', error.message)
  ...
  const proposalItems = proposals.map(p => {
    ...
    return {
      ...
      changeSummary,
      relativeTime: relativeTime(p.created_at as string),
      proposedBy: (p.proposed_by as string) ?? 'unknown',
    }
  })
  ...
  for (const approverEmail of APPROVERS) {
    const email = renderDailyStandardDigest({
      approverEmail,
  ...
  await supabaseAdmin.from('audit_log').insert({
    action: 'cron_standard_digest_sent',
    summary: `Standard proposals digest sent to ${APPROVERS.length} approver(s) — ${proposals.length} proposal(s)`,
